    ...
    if (response.secondary_archetype && process.env.ANTHROPIC_API_KEY) {
      try {
        combinationAnalysis = await generateCombinationAnalysis(
          ...
          ARCHETYPE_NAMES[response.primary_archetype] ?? response.primary_archetype,
          response.secondary_archetype,
          ARCHETYPE_NAMES[response.secondary_archetype] ?? response.secondary_archetype
        );
        await supabase.from('responses').update({ combination_analysis: combinationAnalysis }).eq('id', resultId);
      } catch (err) {
    ...
    if (appPassword) {
      try {
        ...
        await transporter.sendMail({
          ...
          to: response.email,
          subject: `Your Deep Report — ${response.primary_archetype}${response.secondary_archetype ? ' + ' + response.secondary_archetype : ''}`,
          html: buildEmail(response.primary_archetype, response.secondary_archetype ?? null, combinationAnalysis, reportUrl),
        });
